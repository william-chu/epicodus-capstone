...
export default function TrackSubmit(props) {
  ...
  let resultText;
  let suspectHeader = (
    <Text style={styles.h3}>Meals eaten 30-40 hours ago:</Text>
  );
  let resultSuspectMeals;
  let actionText = (
    <Text style={styles.p}>
      View <Text style={styles.strong}>Analyze</Text> screen for additional
      information
    </Text>
  );
  ...
  if (scaleInput !== 3 && scaleInput !== 4) {
    let suspectMealsToDisplay = displaySuspectMeals(
      props.screenProps.masterMealLog,
      mealLogDateKey,
      time
    );
    resultSuspectMeals = (
      <View>
        <Text style={styles.h1}>
          {suspectMealsToDisplay[0]} - {suspectMealsToDisplay[1]}
        </Text>
        <Text style={styles.h4}>{suspectMealsToDisplay[2].join(", ")}</Text>
        <Text style={styles.h1}>
          {suspectMealsToDisplay[3]} - {suspectMealsToDisplay[4]}
        </Text>
  ...
  if (scaleInput === 1 || scaleInput === 7) {
    ...
    specialText = (
      <Text style={styles.redText}>
        If your issues persist, please seek professional medical attention.
      </Text>
    );
